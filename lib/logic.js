...
/**
 * Use this function to award funds to any or all of the grantees
 * @param {com.usgov.ed.grants.AwardFunds} awards
 * @transaction
 */
async function awardFunds(awards){
    const actionRequestAwards = [];
    
    for (const _award of awards){
        if(_award.date == undefined){
            _award.date = new Date();
        }
        const newActionRequest = factory.newResource(NS, 'ActionRequest', GetActionRequestDateID(_award.grantee.getIdentifier(),_award));
        newActionRequest.status = "APPROVED";
        newActionRequest.type = "AWARD";
        newActionRequest.requestValue = _award.amount;
        ...
        newActionRequest.createdDate = _award.date;
        newActionRequest.owner = _award.grantee;
 ...
 */
 async function addValidatingGrantees(validatorParams){
    ...
    const arRegistry = await getAssetRegistry(NS+'.ActionRequest');
    var currentReq = await arRegistry.get(validatorParams.request.getIdentifier().toString());

    // Make sure the action request does not already have validators selected
    if(currentReq.assignedValidators && currentReq.assignedValidators.length === 0){
        // Get the id of the grantee who made the request
        ...
        // Create an array to hold the selected grantees
        const _approvers = [];
    
        const numValidators = validatorParams.validators;
    
        // Ensure that the total number of approvers validators selected are not more than the total number of grantees
        const _numberOfApprovers = validatorParams.validators > allGrantees.length ? allGrantees.length : validatorParams.validators;
    
        for (var i=0; _approvers.length < _numberOfApprovers; i++){
            // Select the address of a random grantee
            var id = Math.floor(Math.random()*allGrantees.length);
            var _selectedApprover = allGrantees[id];
            // Make sure the selected approver is not the grantee who initiated the transaction and that you are not selecting the same approver twice
            if(_selectedApprover.getIdentifier() !== ownerId && _approvers.map(x => {return x.getIdentifier()}).indexOf(_selectedApprover.getIdentifier()) === -1){
                // If the conditions are met, push the new approver to the array
                _approvers.push(_selectedApprover)
            }
        }

        currentReq.assignedValidators = _approvers;
        currentReq.status = "VALIDATORS_SELECTED";

        await arRegistry.update(currentReq);
    } 
    // throw an error if the current action request already has validators
    else {
